...
The dimension reduction in this study is carried out using *DistributionalSemanticsR* [@sevenants_distributionalsemanticsr_2023], another library made for this study. The library is a wrapper around *vegan* [@oksanen_vegan_2022], *Rtsne* [@krijthe_rtsne_2015] and *umap* [@konopka_umap_2022] and facilitates the dimension reduction process. We reduced the dimensions to 2 in order to be able to inspect the distance between the vectors visually. For a visual representation of the semantic space processed by the UMAP dimension reduction technique, see @fig-basic-coords-umap. The plots for the other dimension reduction techniques are available in the appendix.
...
```{r #fig-basic-coords-umap}
#| fig-cap: Visual representations of the semantic space in UMAP
#| column: screen-inset-shaded
#| layout-ncol: 1
#| fig-width: 5
#| fig-height: 5

basic_umap_plot %>% print()
```
...
```{r #fig-pam-r2-umap}
#| fig-cap: Overview of the R^2^ values for each $k$-model for UMAP
#| column: screen-inset-shaded
#| layout-ncol: 1
...
```{r #fig-lm}
#| fig-cap: Visualisation of the regression model predicting the elastic net coefficients based on the semantic space, UMAP (non-eliminated types only)
#| column: screen-inset-shaded
#| layout-ncol: 1
#| fig-width: 6
#| fig-height: 5
...
```{r #fig-gam}
#| fig-cap: Visualisation of the GAM model predicting the elastic net coefficients based on the semantic space, UMAP (non-eliminated types only)
#| column: screen-inset-shaded
#| layout-ncol: 1
#| fig-width: 6
#| fig-height: 5
...
It is immediately clear that a GAM model is much better at capturing patterns in the semantic space. We also see general semantic areas appearing, where the model predicts data points to generally be more red or more green.[^effect-notice]. Still, if we look at the actual data points, the semantic space remains quite messy. Nonetheless, it seems that the GAM models are much better at dealing with semantic noise than the clustering algorithms.

[^effect-notice]: The potential of GAM models is especially obvious for the MDS and TSNE plots, which can be found in the appendix.
...
```{r #fig-gam-squares-umap}
#| fig-cap: "Visualisation of the GAM model predicting the elastic net coefficients based on the semantic space, with subdivision squares and significantly differing squares indicated, UMAP (non-eliminated types only). Each line connects areas with significantly differing adjectiveness values."
#| layout-ncol: 1
#| fig-width: 6
#| fig-height: 5
...
#| fig-cap: Visual representations of the semantic space in MDS and TSNE
#| column: screen-inset-shaded
#| layout-ncol: 2
#| fig-width: 5
#| fig-height: 5
#| fig-subcap:
#|   - "Visual representation of the semantic space, MDS dimension reduction"
...
```{r #fig-pam-r2-extra}
#| fig-cap: Overview of the R^2^ values for each $k$-model for MDS and TSNE
#| column: screen-inset-shaded
#| layout-ncol: 2
#| fig-subcap:
