...
Next, we will compare our elastic net coefficients to the odds ratios (ORs) found in @bloem_processing_2021 TODO. We applied the same transformation here, making all ORs in favour of the green word order negative, in order to be able to compare these OR values with the elastic net coefficients. Of the `r bloem_total_count %>% formatn` participles in Bloem's semantic analysis, `r bloem_zero_count %>% formatn` (`r bloem_zero_perc`) were eliminated in the elastic net model. Here again, this means that the elastic net technique finds no semantic pull, again likely due to differences in operationalisation. An overview of the eliminated coefficients is given in @tbl-comparison-bloem-zero.

```{r #tbl-comparison-bloem-zero}
#| tbl-cap: "Overview of all participles which have OR values, but were eliminated in our elastic net regression model" 
bloem_zero <- bloem_zero[c("feature","coefficient","oddsr")]
# Re-order
bloem_zero <- bloem_zero[,c(1, 3, 2)]
bloem_zero <- bloem_zero[order(bloem_zero$llr, decreasing=FALSE),]
colnames(bloem_zero) <- c("participle", "odds ratio", "elastic net coefficient")
row.names(bloem_zero) <- NULL
bloem_zero
```

There are also `r bloem_na_count %>% formatn` (`r bloem_na_perc`) participles which do not appear in our dataset, and therefore do not have an associated coefficient. TODO want raar

```{r #tbl-comparison-bloem-na}
#| label: tbl-comparison-bloem-na
#| tbl-cap: "Overview of all participles which have OR values, but do not appear in our dataset and therefore do not have coefficients" 
bloem_na <- bloem_na[c("feature","coefficient","oddsr")]
bloem_na$coefficient <- "none"
# Re-order
bloem_na <- bloem_na[,c(1, 3, 2)]
bloem_na <- bloem_na[order(bloem_na$llr, decreasing=FALSE),]
colnames(bloem_na) <- c("participle", "odds ratio", "elastic net coefficient")
row.names(bloem_na) <- NULL
bloem_na
```

We left out one participle ('gezegd') since it had an unusually high OR value, and then computed the Pearson correlation between the modified OR values and our elastic net coefficients. We found a correlation $r$ of `r bloem_cor$estimate %>% formatd(3)`, which again is significant. The correlation shows a strong overlap between the OR results and our results, despite differences in methodology. 

```{r #fig-comparison-bloem}
#| fig-cap: "A visual comparison of the OR values and our elastic net coefficients" 
quadrant_plot(df_bloem$oddsr, df_bloem$coefficient, "Odds ratio", "Elastic net coefficient")
```

If we look at the correlation visually (@fig-comparison-bloem), we again see that there are virtually no data points in the top left and bottom right quadrants, which means there is high agreement with @bloem_processing_2021 on which participles are red and which participles are green.
...
As a basis for our semantic analysis, we used the 'count' snaut vectors for Dutch [@mandera_explaining_2017] -- a ready-made package with semantic vectors for almost any Dutch word imaginable. These vectors are constructed per word form (they are not constructed per attestation), which means that homonyms receive a single vector. Still, we did not expect a high level of homonymy for participles and thus did not expect issues for our study.

One of the other properties of semantic vectors is that they are high-dimensional. For example, a snaut vector has a length of 200, which means that a single word is represented by 200 numbers. If we want to inspect the vector space of our words visually, we have to reduce the vectors to two or three dimensions. Typically, this is done by first computing a distance matrix, i.e. a matrix which shows the pairwise distances between each word, and then applying dimension reduction to that matrix. The dimension reduction technique will attempt to capture the information found in the high-dimensional space as well as possible in the lower-dimensional space. While there are several dimension reduction techniques available, we chose MDS [@douglas_carroll_multidimensional_1998], TSNE [@hinton_stochastic_2002] and UMAP [@mcinnes_umap_2020], following @montes_cloudspotting_2021.

The dimension reduction in this study is carried out using *DistributionalSemanticsR* [@sevenants_distributionalsemanticsr_2023], another library made for this study. The library is a wrapper around *vegan* [@oksanen_vegan_2022], *Rtsne* [@krijthe_rtsne_2015] and *umap* [@konopka_umap_2022] and facilitates the dimension reduction process. We reduced the dimensions to 2 in order to be able to inspect the distance between the vectors visually. For a visual representation of the semantic space processed by the MDS dimension reduction technique, see @fig-basic-coords-mds. The plots for the other dimension reduction techniques are available in the appendix.
...
The PAM (Partitioning Around Medoids) technique [@kaufman_partitioning_1990] selects a predefined number of data points as cluster centres and then builds clusters around those central points. We used the R implementation in the `cluster` package [@maechler_cluster_2022, `pam` function]. Because a predefined number of clusters is required and we have no prior knowledge on the desirable number of clusters, we built clusterings with a number of clusters ranging from 2 to 40. The upper limit is a practical choice made to avoid compute issues at larger cluster numbers.
