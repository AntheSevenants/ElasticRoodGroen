...
|valence category|whether the verb is rated as 'neutral', 'positive' or 'negative'|
...
semset_fit <- lm(
  coefficient ~ control + attributive + spatial + cognitive + dynamic + valency + ValenceCategory,
  data = df_semset
)
...
The regression results show the differences to the elastic net coefficient of a verb when one of these elements changes. We see that there are no significant differences, except for when a verb demands emotional, perceptual or mental activitity (tagged as 'cognitive'). For these verbs, a logit adjustment needs to be made towards the green order, though it is rather small (`r semset_fit_tidy %>% filter(term == "cognitiveTrue") %>% .$estimate %>% formatd(3)`). Do note that this predictor is only borderline significant.
...
Starting from a neutral reference point, verbs with an emotional load (either positive or negative) lead to a slight shift towards the green word order (`r semtype_fit_tidy %>% filter(term == "valenceemotional") %>% .$estimate %>% formatd(3)`).
...
Based on the results of the analyses of the Cornetto lexical semantic database, we can also say that the choice between the red and green word order is at least partially syntactically and semantically motivated. The analysis of the narrow semantic labelling hints that, like @bloem_processing_2021 concluded on the basis of his small sample, verbs with a cognitive dimension tend, albeit very slightly, towards the green word order. The broader analysis showed another skew towards green with verbs with an emotional load, comparable to the pattern found by @bloem_processing_2021 with predominantly negative verbs.
