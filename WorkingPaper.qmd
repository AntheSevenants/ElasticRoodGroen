---
title: "Investigating lexical-semantic effects on the red and green word order in Dutch using elastic net regression and generalised additive models"
author:
...
Lexical-semantic effects have played an important role in linguistics for a long time. Even in very formal theories of language, the lexicon is assigned a special position. An example of this is found in Syntactic Theory [@sag_syntactic_2003], where the lexicon is specifically encoded in the theory of syntax. Indeed, the authors mention (131):
...
Of course, in this view the lexicon is otherwise separated from syntax, but the importance of lexis remains the same. Most famously, construction grammar goes one step further and forfeits the border between lexis and syntax entirely. Lexical material and syntactic constraints appear on the same level and are mixed together in a "lexicon-syntax continuum" [@hoffmann_construction_2013, 1]. This is by design, since the construction grammar architecture can "account for the entirety of language instead of making a distinction between core linguistics \[...\] and \[...\] exceptions" [@beuls_computational_2016 5]. While it is clear that there is a lot of interest for the lexicon and its relationship with syntax, and some efforts have been made to investigate lexical-semantic effects, no 'real' methodology exists yet. This is in part due to the fact that it is very hard to research semantic effects quantitatively.

Currently, there are two ways in which the effect of semantics on morphosyntactic alternances is measured. The first way is through annotations of one specific semantic feature. For example, in @nevalainen_probabilistic_2008, attestations of the *of*- or *s*-genitive are coded for animacy of the possessor. There are three problems with this approach. The first is that this way of measuring semantic effects only highlights one aspect of the semantic value of lexical items. In addition, one can typically only measure the *general* effect of this semantic feature; there are no per-word results. Finally, since annotations hinge on semantic estimations by humans, or are limited to the perception of meaning by humans, bias is unavoidable in such studies. In theory, one could extract the value of a semantic feature automatically, thus avoiding bias to a certain extent, but the selection of the feature and the retrieval process are still biased by a human view on semantics.[^bias]

[^bias]: For example, if the automatic detection of a semantic feature is done through a computer program, the logic of the computer program will be influenced by the semantic bias or views of the human who wrote that program.

The second way in which semantic effects on morphosyntactic alternances are measured is by analysing the general semantic 'pull' of lexical items towards the opposing constructions, i.e. by measuring the frequency distribution of the lexical items. An early example is @tummers_quantifying_2004, who use the log-likelihood ratio in a regression analysis to investigate the lexical specificity of opposing constructions. One option explored by @bloem_processing_2021 is to investigate the residuals of each data point in a logistic regression. The idea is that the mean residuals for a specific type (e.g. the residuals for all data points with the same word) hint at the lexical association of that type towards an option of a specific alternance [@bloem_processing_2021 118, 135]. While this approach to measuring lexical-semantic effects also works, it lumps together unexplained variation and lexical-semantic effects into one category, which is far from ideal.

@bloem_processing_2021 also attempted to use collostructional analysis to investigate the lexical pulls on a syntactic alternance. While this technique works and indeed gives us the lexical pulls of each participle individually, it does not feature multifactorial control. This means it is difficult to focus purely on the lexical semantic effects, as there can be interference from other (un)known factors. While Bloem attempts to mitigate this issue by using Gain Ratio, a measure which quantifies how much a baseline multifactorial model *without* lexical information improves when lexical-semantic information is added, Bloem admits that this methodology is not ideal (117): 

> This may sound like a more cumbersome way of calculating the effect of a random factor in a multifactorial regression model, and it is.

Indeed, a current state-of-the-art analysis of lexical effects would attempt to use random effects in a multifactorial logistic regression [cf. @gries_most_2015], with the lexical type of each data point as a random effect. This would result in a correction term for each type separately, which would indicate how specific words contribute towards the choice between two constructions. However, as @van_de_velde_investigating_2019 [168-169] describe, there are generally two issues with this approach. The first issue is that using random effects in this way is, strictly speaking, conceptually unsound. Random effects are 'asides' in a model, meant to account for structural associations in the data. They are used when you want to correct for intra- or extra-linguistic influences which are not part of your analysis, but for which you nonetheless want to correct. In the case of lexical effect research, putting the main focus of your analysis, verb types, in the random effects simply does not make sense conceptually. Ideally, these verb types should be part of the so-called *fixed effects*, which are at the basis of the actual analysis.

This brings us to the second issue raised by @van_de_velde_investigating_2019; there are *many* possible lexical types, and word distribution is notoriously sparse [@zipf_psycho-biology_1965]. As @bloem_processing_2021 [117] also says, this results in "an impossible task when there are many unique \[words\] -- you can't create a line from a single data point" -- and models will fail to converge given too many lexical types. A possible solution to this problem of high dimensionality would be to use some sort of 'semantic binning', i.e. by grouping individual words into larger semantic categories. However, this means we lose the ability to look at lexical effects at the level of individual words, and also introduces bias into the analysis at the level of the grouping itself.

In this article, we demonstrate a next-generation methodology for researching lexical-semantic effects on morphosyntactic alternances through the use of elastic net regression. The results of this technique will be combined with distributional semantic vectors, clustering algorithms and General Additive Models (GAMs) in order to gain more general semantic insights. First, however, we will explain what the Elastic Net technique does and why it is so useful for lexical-semantic research.
...
We included frequency information from the SUBTLEX dataset [@keuleers_subtlex-nl_2010] in order be able to assess the effect of frequency. Because frequency is typically Zipfian [@zipf_psycho-biology_1965], we transformed the frequency information using the natural logarithm in order to compress the frequency variation among the types in our dataset.
...
It is immediately clear that the GAM models are much better at capturing patterns in the semantic space. Interestingly, we also see general semantic areas appearing, where the model predicts data points to generally be more red or more green. Still, if we look at the actual data points, the semantic field remains quite messy. Nonetheless, it seems that the GAM models are much better at dealing with semantic noise than the clustering algorithms.

With the affirmation of the existence of these red and green lexical areas, the question arises *what* verbs we find in these areas. What semantic properties make a verb particularly red or green? To investigate contents of the different semantic areas, we subdivided the semantic space into nine square subareas. In the top-left corner of each square, we show the number of data points in that square (first line) and the mean adjectiveness of the points in that square (second line). Each square is assigned a so-called 'dominant colour'. This dominant colour is based on a t-test of the coefficients of the data points in that square. The null hypothesis for each square is that the coefficients do not specifically pull towards either word order. If significance was not attained in the t-test, the square receives a grey outline to indicate the lack of a dominant colour. If significance was attained in the t-test for that square, we assumed a dominant colour on the basis of the t-test estimate's sign; a negative estimate shows a square's pull towards the green order, a positive estimate shows a pull towards the red order. @fig-gam-squares shows an overview of the squares and their dominant colours (the coloured lines will be explained shortly).
...
#| fig-height: 5
#| fig-subcap:
#|   - "Visualisation of the GAM model predicting the elastic net coefficients based on the semantic space, with subdivision squares and significantly differing squares indicated, MDS (non-eliminated types only)"
#|   - "Visualisation of the GAM model predicting the elastic net coefficients based on the semantic space, with subdivision squares and significantly differing squares indicated, TSNE (non-eliminated types only)"
#|   - "Visualisation of the GAM model predicting the elastic net coefficients based on the semantic space, with subdivision squares and significantly differing squares indicated, UMAP (non-eliminated types only)"
