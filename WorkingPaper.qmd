...
The issue with these two lines of research is that they inadvertently have a bias for one specific type of predictor. Regression analysis promotes the use of high-level, clear-cut predictors, while collostructional analysis is solely focussed on lexical items. This puts these probabilistic models into a pickle, as they are forced into highlighting only part of the phenomena under scrutiny. Of course, this also has repercussions theoretically, as predictors of different natures are never present as equals in the same models, which means linguistic conclusions are usually only centred around one or the other -- never both at once. Of course, these results are still valid in the strict sense, but they never paint a full picture.
...
To demonstrate the elastic net technique, we built an elastic net model of red and green verb clusters in Dutch. The focus in this study lies on two-word verbal clusters in subordinate clauses with the auxiliaries *hebben*, *zijn* and *worden* (perfective aspect + passive auxiliary). Some variation is also possible with modal auxiliaries, but these mostly appear in the red order, which makes them much less interesting.

For our model, we collected all red and green verb clusters in subordinate clauses in the SoNaR Corpus [@oostdijk_d-coi_2008] and SoNaR New Media Corpus [@oostdijk_sonar_2014]. For more information about the data collection process and the annotation of linguistic variables, see the Appendix.
