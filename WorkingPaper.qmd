...
bibliography: references_latex.bib
toc: true
toc-depth: 4
toc-location: left
...
Whenever there are "alternate ways of saying the same thing" [@labov_sociolinguistic_1972, 188] in a language, linguists take great interest. As such, several approaches have been developed to try and explain the choice between these "alternate ways" as accurately as possible. Generally speaking, we can distinguish between three broad methodological lines to approach research on morphosyntactic variation. The first methodological line is characterised by purely formal accounts. In these analyses, the cause of variation is sought in language itself, or in microparameters that are explained on a regional basis [@adger_variation_2007; e.g. @van_craenenbroeck_quantitative_2019]. Morphosyntactic variation is then built into the syntax tree itself, or in some post-syntactic process.

The second methodological line is characterised by a focus on high-level, general predictors, stemming from actual language use. In such analyses, we find predictors with a limited number of levels, sometimes pooled from more narrow categories. Theoretically, they align most with "probabilistic grammar", the idea that "variation [...] is sensitive to multiple and sometimes competing constraints which influence linguistic choice-making" [@grafmiller_general_2018, 1]. To discover these constraints, (mixed-effects) regression analysis [e.g. @gries_most_2015] is typically used.

The third methodological line is characterised by a focus on one specific aspect of language use: frequency, and frequency only. In such analyses, the pull of lexical items towards a specific morphosyntactic variant is computed. This practice fits within the larger theoretical framework of Construction Grammar (CxG): as CxG advocates for a continuum between lexicon and syntax rather than a strict division [@hoffmann_construction_2013] [^formal], multi-word units are promoted to a position of special interest. These multi-word chunks represent supposed semi-conventionalised, intermediate-level cognitive units of language, often abstracted to degrees of underspecification. To discover these constructions and find out the slot-filling tendencies of different words, collostructional analysis [@stefanowitsch_collostructional_2013] is typically used (or derivatives of this technique).

[^formal]: In formal theories of language, the lexicon is often seen as an entity wholly separate from syntax.

The issue with these previous two lines of research is that they inadvertently have a bias for one specific type of predictor. The type of analysis typically carried out with regression analysis promotes the use of high-level, clear-cut predictors, while the lexical frequency research with collostructional analysis is solely focussed on lexical items. This puts these different models into a pickle, as they are forced into highlighting only part of the phenomena under scrutiny. Of course, the choice between these two approaches has its peculiar repercussions, as predictors of different natures are never present as equals in the same models, which means linguistic conclusions are usually only centred around one or the other -- never both at once. Naturally, these results are still valid in the strict sense, but they never paint a full picture.

In this article, we will showcase elastic net regression, a regression technique which can handle both high-level and lexical predictors at once in a single model. We will demonstrate this technique through a case study of a morphosyntactic alternance in Dutch. Later, we will come back to the repercussions of elastic net for CxG as a theoretical concept, and the affordances it brings. First, however, we will discuss previous attempts to bring the lexical dimension into models of variation.
...
In collostructional analysis [@stefanowitsch_collostructional_2013], the lexicon takes the centre stage. The technique can compute precise lemma-level preferences for the choice between two constructions, but it does not feature multifactorial control. This means that it is difficult to focus purely on the lexical influence proper, as there can be interference from other (un)known factors that cannot be taken into account in the model.
...
We saw earlier that lasso and ridge regression both have their own strengths. Elastic net regression leverages those strengths and *combines* both techniques. This means that both L1 and L2 regularisation are used at the same time, with an additional parameter alpha ($\alpha$) regulating the split between lasso and ridge penalty. @eq-elastic-net shows this [adapted from @friedman_regularization_2010].
$$
\min_{(\beta_0, \beta) \in \mathbb{R}^{p+1}} \underbrace{ - \biggl[ \frac{1}{N} \cdot \sum_{i=1}^N y_i \cdot (\beta_0 + x_i^T \cdot \beta) - \log (1+e^{(\beta_0+x_i^T \cdot \beta)}) \biggr] }_{\text{negative log-likelihood of logistic regression}} +
...
Theoretical-formal accounts based on movement, however, have difficulty finding an explanation for the occurrence of either the red or green word order. While there are ample theoretical devices to derive any word order from any base order, even for highly complex verbal clusters, there are no theoretical grounds for explaining the possible variation between red and green. Crucially, if the choice between two variants is free, there cannot be a trigger for movement, yet movement is compulsory in order to be able to derive the non-base word order. [@barbiers_merging_2018] avoid this issue by advocating for a Merge operation rather than movement, as Merge does not entail a specific order by itself. The difference between both orders is also resolved outside the syntax tree, just like for Flip, when the tree is linearised. Whether a verb cluster is linearised leftward (red word order) or rightward (green word order) is, according to them, dictated on geographical terms. While this formal solution is elegant, it ignores the supposed freedom of choice that is granted to language users, regardless of their geographical location.[^formal-ignores-individual]

[^formal-ignores-individual]: Of course, theoretical-formal accounts *by design* do not engage deeply with the individual language user. We therefore regard  "skipping" the individual as a side effect of the theory, rather than a flaw.

### Probabilistic and lexical accounts {#sec-probabilistic-accounts}

Usage-based studies of the red-green phenomenon are not subject to the aforementioned theoretical movement issues, and do not root the origin of word order variation strictly in geography. Instead, they attempt to explain verb cluster variation using probabilistic models (see @sec-introduction). In this section, we will give an overview of the different probabilistic influences that have been found so far for the red and green word order.

#### Influence of lectal factors

"Lectal" factors [@geeraerts_lectal_2005] are an important influence in the choice between the red and green word order, with a general distinction between two types of contexts. The first is the *regional* context: @de_sutter_rood_2005 [68] found that the red word order is much less preferred in Belgium than in the Netherlands, a result corroborated by @bloem_processing_2021 [206]. The second context is register: in spoken, interactional and unedited language use, the green order is much more prevalent [@de_sutter_rood_2005, 81-87]. Conversely, formal, unidirectional and edited language favours the red word order.

#### Influence of prosody

Another source of influence is that of prosody. The most important prosodical influence [@de_sutter_rood_2005, 305] stems from the so-called "separable verbs" in Dutch. These verbs, comparable to phrasal verbs in English, have a preposition which can be detached from the root.[^separable-example] These word-initial prepositions always carry stress, giving rise to a chain of two adjacent stresses. @de_sutter_rood_2005 [154] found that speakers often choose the red order over the green order in order to avoid this chain. Consider @exm-prosody-green.
...
If both *definitief* 'definitively' and *afgedwongen* 'enforced' are stressed, as they would typically be in Dutch, there would be two consecutive stresses. If one uses the red word order, this chain of stresses can be avoided (@exm-prosody-red).
...
@de_sutter_rood_2005 [289-295] found that the frequency of a participle's lemma positively correlates with the probability of that participle appearing in the red order. @bloem_processing_2021 [24] repeated the corpus study and also found an effect in the same direction. @de_sutter_rood_2005 [323-324] argue that frequently appearing participles are easier to recover from memory, leaving more time for the language user to spend on (more difficult) stylistic encoding, which @de_sutter_rood_2005 then assume is the red order. 
...
After filtering invalid occurrences, `r nrow(df_filtered) %>% formatn` attestations of the red and green word order remained, belonging to `r types_count %>% formatn` unique types.[^types] `r subset(df_filtered, order == "red") %>% nrow %>% formatn` attestations belong to the red order, `r subset(df_filtered, order == "green") %>% nrow %>% formatn` belong to the green order. `r subset(df_filtered, country == "BE") %>% nrow %>% formatn` attestations come from Belgium, `r subset(df_filtered, country == "NL") %>% nrow %>% formatn` attestations come from the Netherlands. The imbalance between Belgium and the Netherlands stems from an overall imbalance between Belgian and Netherlandic material in SoNaR. This highlights why it is so important to have a multifactorial technique, like elastic net regression, which can account for these kinds of issues. In a collostructional analysis, such variation would influence the results.
...
For an overview of the proportions and distributions of the high-level predictor values, see @fig-descriptive-plots. The operationalisation of our high-level predictors is available in @sec-filtering-enriching and @sec-converting-dataset. The mean adjectiveness among all attestations is `r df_filtered$adjectiveness %>% mean() %>% formatd()` (median `r df_filtered$adjectiveness %>% median() %>% formatd()`). 
...
| predictor | explanation | type | possible values |
| --- | --- | --- | --- |
| participle | the participle in the verbal cluster | distributed binary values for each type | in verb cluster/not in verb cluster |
| auxiliary type | the type of the auxiliary in the verbal cluster | distributed binary values for each auxiliary | *zijn*, *hebben*, *worden* |
| country | the country of the text	| binary | BE/NL | 
| separable | whether the verbal cluster contains a separable verb as its main verb | binary | yes/no |
| edited | whether the cluster appears in a text genre considered 'edited' | binary | yes/no |
| adjectiveness | the percentage of adjective use of the participle in the verbal cluster | real number | $0$ to $1$ |
| priming_rate | the value for the measure expressing how primed the verbal cluster is for the red word order | real number | $-\infty$ to $+\infty$ |
| logfreq | the logarithmic frequency of the participle in the verbal cluster | natural number | $0$ to $+\infty$ |
| log_middle_length_field | the logarithmic length of the middle field in the sentence in which the verbal cluster occurs | natural number | $0$ to $+\infty$ |
...
ElasticToolsR automatically finds the appropriate alpha and lambda values which deliver the best possible model. It does this by trying out many different alpha values, and then selecting the alpha value which produces the model with the lowest loss. For each attempted alpha value, lambda is decided using 10-fold cross validation for each attempted alpha value. Our model for the red and green order performs the best at alpha = $`r alpha %>% formatd(0)`$ (so-called "lasso regresion") and lambda = $`r lambda %>% fancy_scientific()`$.
...
Next, we will compare our elastic net coefficients to the odds ratios (ORs) found in @bloem_processing_2021. We converted the ORs back into (continuous) logits, so we could compare them to our elastic net coefficients. Of the `r bloem_total_count %>% formatn` participles in Bloem's semantic analysis, `r bloem_zero_count %>% formatn` (`r bloem_zero_perc`) were eliminated in our elastic net model. Here again, this means that the elastic net technique finds no semantic pull, again likely due to differences in operationalisation. An sample of the eliminated coefficients is given in @sec-comparison-tables, @tbl-comparison-bloem-zero.

There are also `r bloem_na_count %>% formatn` (`r bloem_na_perc`) participles which do not appear in our dataset, and therefore do not have an associated coefficient. Some of these participles seem to be mis-taggings or noise. We again provide a sample in @sec-comparison-tables, @tbl-comparison-bloem-na.
...
For completeness' sake, we also provide Bayesian estimates for the correlations between our coefficients and the other values. These are provided in @sec-bayesian-correlations, @tbl-comparison-ds-bayes and @tbl-comparison-bloem-bayes.
...
Verbs in Cornetto can have more elaborate semantic tags which describe the specific senses of verbs in further detail. There are two types of these semantic tags: broad and narrow tags. A broad semantic tag describes a verb as either a "process", "state" or "action". A narrow semantic tag is more elaborate and consists of five possible attribute values, which can either be present or not. For example, *herstellen* 'to repair' is tagged broadly as `action`, and more narrowly as `+Control`. Ideally, all verbs would be tagged with the most elaborate attributes, but unfortunately this is not the case. Of the `r types_count %>% formatn` types in our dataset, only `r cornetto_size %>% formatn` (`r cornetto_size_perc`) are part of the Cornetto database; `r cornetto_broad_size` (`r cornetto_broad_size_perc`) have broad semantic tags, just `r cornetto_narrow_size` (`r cornetto_narrow_size_perc`) have narrow semantic tags. While all types with narrow semantic tags also have broad semantic tags, this is not the case in the other direction, presumably because minutely tagging hundreds of words was beyond the scope of the Cornetto project. Nonetheless, will build analyses with both broad and narrow semantic properties.
...
As mentioned before, only a subset of verbs is tagged for specific semantic properties, with the more elaborate semantic tags being even fewer. This unfortunately limits the number of types that can be used for a lexical-semantic analysis. However, we still believe there is value in carrying out an analysis with this smaller subset of the data, as it not only provides a more interpretable window into possible semantic patterns, but also allows us to replicate the research by @bloem_processing_2021, as mentioned previously. To not have to choose between statistical power and annotation resolution (the broader tags are more numerous, the more detailed tags are fewer), we decided to make two analyses. In the fine-grained analysis, we used the smallest subset of the data with the richest annotations, while in the coarse-grained analysis, we used the broader "semantic type" predictor available for more types.
...
In our fine-grained analysis, based on the smaller dataset, we modelled the elastic net coefficients of all verbs using the variables from @tbl-variables as predictors in a linear regression model. In short, we wanted to model whether the presence of specific semantic properties (i.e. the subject of the verb being capable of acting with volition, the verb expressing a changing situation ...) influences the red or green preference of verbs. @tbl-semset-regression shows the results of this regression model. Keep in mind that the results need to be interpreted with regards to the reference situation expressed as the "Intercept". In our case, this reference situation is:
...
The regression results show the differences to the elastic net coefficient of a verb when one of these elements changes. Only adjustments with a p value below 0.05 have sufficient statistical evidence. We see that for verbs which are valenced (having either positive or negative associated emotions), there needs to be a small adjustment. The value for "Valence: Valenced" under the "Estimate" column (`r semset_fit_tidy %>% filter(term == "valencevalenced") %>% .$estimate %>% formatd(3)`) shows this correction quantitavely. Like before, the negative signs indicates a correction towards the green word order, which in turn means that verbs with a certain emotional loading tend slightly more towards the green word order. Of all semantic properties (i.e. control, attributive, spatial ...), we see that the "cognitive" property reaches closest to significance, though it does not attain it. This is different from the analysis by @bloem_processing_2021, where verbs tagged "cognitive" *did* attain significance.
...
With this larger dataset size, we see that semantics- and valency-related predictors now reach significance. In fact, all predictors have p values below 0.05. We will treat the different corrections one by one.

1. Valency

Starting from an intransitive reference point, changes to transitive or ditransitive verbs require a correction towards the green order (`r semtype_fit_tidy %>% filter(term == "valencydi") %>% .$estimate %>% formatd(3)` and `r semtype_fit_tidy %>% filter(term == "valencytri") %>% .$estimate %>% formatd(3)` respectively). This means we see a slight preference for the green word order for these types of verbs. We ran an additional post-hoc analysis [through `emmeans`, @emmeans] to determine whether there are mutual differences between the verb types. @tbl-valency-posthoc shows that significant differences exist only between intransitive and ditransitive verbs, and intransitive and ditransitive verbs. There are no mutual differences between transitive and ditransitive verbs.
...
Like in the fine-grained analysis, starting from a neutral reference point, verbs with an emotional load (either positive or negative) lead to a slight shift towards the green word order (`r semtype_fit_tidy %>% filter(term == "valencevalenced") %>% .$estimate %>% formatd(3)` in @tbl-valency-posthoc). We thus see the same slight green preference for emotionally loaded verbs.

3. Semantic type

Our reference situation expresses a "state" semantic type. We see that there is a significant difference between this state reference type and the "no state" type consisting of both "action" and "process". We see that when the semantic type changes to not a state, the elastic net coefficients shift slightly towards the green order (`r semtype_fit_tidy %>% filter(term == "semantic_typeno_state") %>% .$estimate %>% formatd(3)`). Interestingly, then, verbs which do *not* express a state, seem to bias slightly towards the green word order.
...
First of all, our clusters of verbs, based on a bottom-up clustering of semantic vectors, show that there are more semantic areas in our semantic space which are predominantly green. While it is true that our results contain more green coefficients in the first place, the split between predominantly red and green clusters in our bottom-up clusters is much more skewed than the split between red and green verb types found in our dataset (see @sec-results-lexical). On the basis of this outcome, one could say that this result corroborates the idea by @de_sutter_rood_2005 that the green word order is more lexically specific, since it clearly captures a larger proportion of the semantic space. We also saw this in the range of coefficients in general in @fig-coefficient-plots-3.
...
In the introduction to this article, we raised several issues about the current leading practices in variationist research taking into account the lexicon. Probabilistic grammar-adjacent research attempts to include lexical variation as part of the random effects structure in a mixed-effects regression model [@gries_most_2015], but this solution is both statistically and conceptually flawed. CxG-centred research lets the lexicon take the spotlight with collostructional analysis [@stefanowitsch_collostructional_2013], but fails to control for other factors.

To the end of theoretical and methodological reconciliation between these two lines of research, we introduced elastic net regression. We presented elastic net regression as a technique to build fairer models of morphosyntactic variation, as it allows lexical items to appear on the same level as traditional, high-level predictors, in contrast to the aforementioned statistical techniques. 
In addition, affordances like the ability to disable certain levels of lexical item predictors, useful  to boost the quality of the model if necessary, make elastic net models robust against typical lexicon-related nuisances like Zipfian-related sparsity issues and overfitting. To demonstrate this, we built a model which attempts to explain the variation between red (auxiliary + participle) and green (participle + auxiliary) verbal clusters in Dutch.

We extracted `r nrow(df_filtered) %>% formatn` verb clusters from the SoNaR corpus [@oostdijk_d-coi_2008] and encoded the most important high-level predictors for each attestation to include in the analysis. We also included the lexical type of each verb cluster in the model as a full predictor, which is the most important innovation in our model. We found that our model yields similar results for both the high-level predictors as well as the lexical associations compared to previous models, which shows that the model serves its intended purpose of delivering reliable results for predictors of different natures in a single model.

While our results show that, methodologically, elastic net regression works as expected, there are also theoretical repercussions. First of all, the different coefficients computed by the model for the various verbs show that a divide between lexicon and syntax does not make that much sense. Indeed, even when accounting for a plethora of other factors, there is a lot of variation left to be explained in lexical terms. The fact that our coefficients do not all amount to zero proves that the idea of freely casting lexemes into the mold of syntactic constructions, implicit in generative theories, is theoretically undesirable.

At the same time, by introducing a method that takes the lexicon seriously, we bring CxG further than just breaking down the barrier between syntax and lexicon. Currently, much of the research around CxG does not go beyond two basic assumptions: that multiword patterns exist, against the atomic view of the lexicon consisting of just words, and that syntactic constructions are partially filled with concrete lexical material [e.g. @israel_way_1996]. Our provided elastic net technique moves the goalpost in several ways. First of all, it allows one to investigate the role of the lexicon in abstract templates on a much bigger scale than before [expanding upon studies like those by @bossuyt_oppassen_2019; @colleman_verb_2009]. In addition, it also operates on a highly quantitative level, and does this under multifactorial control. As such, elastic net is an excellent tool for researchers interested in lexical effects as part of a more elaborate and intensive CxG framework.

While our study has shown the feasibility and usefulness of a monolithic elastic net model, there are still further steps to be taken in order to arrive at a deeper understanding of the influence of the lexicon on morphosyntactic variation. Indeed, though our model shows that lexical influence exists, we do not have an all-encompassing explanation for all lexical patterns we see in our specific case study. The vector-based clusterings show semantic structures, but are difficult to interpret; the regression analyses based on the lexical-semantic database are interpretable, but their effects are small and the results are partially plagued by data scarcity. Perhaps many of the lexical patterns we find are too local to appear in our analysis, or we need to find other possible explanations of the variation we see in future research, e.g. phonological similarity.

The difficulty in finding clear-cut patterns might also be the explanation itself. Indeed, the lexical specificity of a language can function as an aspect that is hard to master, and therefore has value as a social marker. As @hurford_origins_2014 [49] states, "the complexity of languages is a signal of group membership". In their computer simulation on human cooperation, @nettle_social_1997 have shown that it is beneficial to devise an adequately complex "distinctive code" [-@nettle_social_1997 98] in order to provide stability in large social groups. The lexical patterns we find here could be an example of such a social marker and distinctive code. We leave this evolutionary speculation to further research.

Specifically for future elastic net regression research, it would be interesting to apply the elastic net technique to other morphosyntactic alternations, especially in other languages, to see how it holds up in these other contexts. This would also help address several of the questions remaining around the technique and the possible post-analyses. In this way, we will learn more about how language users choose between equivalent variants, and in what way the lexicon contributes towards this choice.
...
**Funding:** We are grateful to Fonds Wetenschappelijk Onderzoek (G059922N) for supporting this study.

## Appendix {#appendix .appendix}

### Try it yourself

To aid the analysis of the elastic net coefficients, a Javascript-based interactive analysis tool, Rekker, was developed [@sevenants_rekker_2023]. You can look at the dataset and elastic net results yourself by visiting [anthesevenants.github.io/Rekker/](anthesevenants.github.io/Rekker/).
...
### Filtering and enriching {#sec-filtering-enriching}
...
: Example input matrix to illustrate the workings of elastic net regression. {#tbl-example-input-matrix}
...
### Bayesian correlations {#sec-bayesian-correlations}

To provide more robust evidence for the correlation between our elastic net coefficients and the results of previous studies, we have also computed the Bayesian correlations between the two, complete with their credible intervals (CI), according to @van_doorn_bayesian_2018. The results are given in @tbl-comparison-ds-bayes and @tbl-comparison-bloem-bayes.

| Measure | Estimate | CI |
| --- | --- | --- |
| Pearson | `r ds_cor_bayes$Median %>% formatd(3)` | `r ds_cor_bayes$CI_low %>% formatd(3)`-`r ds_cor_bayes$CI_high %>% formatd(3)` |
| Kendall | `r ds_cor_ken_bayes$r %>% formatd(3)` | `r ds_cor_ken_bayes_ci$lowerCI %>% formatd(3)`-`r ds_cor_ken_bayes_ci$upperCI %>% formatd(3)` |
: The Bayesian correlation results for the comparison between De Sutter's LLR values and our elastic net coefficients {#tbl-comparison-ds-bayes}

| Measure | Estimate | CI |
| --- | --- | --- |
| Pearson | `r bloem_cor_bayes$Median %>% formatd(3)` | `r bloem_cor_bayes$CI_low %>% formatd(3)`-`r bloem_cor_bayes$CI_high %>% formatd(3)` |
| Kendall | `r bloem_cor_ken_bayes$r %>% formatd(3)` |`r bloem_cor_ken_bayes_ci$lowerCI %>% formatd(3)`-`r bloem_cor_ken_bayes_ci$upperCI %>% formatd(3)` |
: The Bayesian correlation results for the comparison between Bloem's OR values and our elastic net coefficients {#tbl-comparison-bloem-bayes}

### Comparison tables {#sec-comparison-tables}

The following tables show the inconsistenties between our results and those of @de_sutter_rood_2005 and @bloem_processing_2021.
