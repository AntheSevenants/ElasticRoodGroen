...
At the same time, by introducing a method that takes the lexicon seriously, we bring CxG further than just breaking down the barrier between syntax and lexicon. Currently, much of the research around CxG does not go beyond two basic assumptions: that multiword patterns exist, against the atomic view of the lexicon consisting of just words, and that syntactic constructions are partially filled with concrete lexical material [i.e. @israel_way_1996]. Our provided elastic net technique moves the goalpost in several ways. First of all, it allows one to investigate the role of the lexicon in abstract templates on a much bigger scale than before [expanding upon studies like those by @bossuyt_oppassen_2019; @colleman_verb_2009]. In addition, it also operates on a highly quantitative level, and this under multifactorial control. As such, elastic net is an excellent tool for researchers interested lexical effects as part of a more elaborate and intensive CxG framework.

While our study has shown the feasibility and usefulness of a monolothic elastic net model, there are still further steps to be taken in order to arrive at a deeper understanding of the influence of the lexicon on morphosyntactic variation. Indeed, though our model shows that lexical influence exists, we do not have an all-encompassing explanation for all lexical patterns we see in our specific case study. The vector-based clusterings show semantic structures, but are difficult to interpret; the regression analyses based on the lexical-semantic database are interpretable, but their effects are small and the results are partially plagued by data scarcity. Perhaps many of the lexical patterns we find are too local to appear in our analysis, or we need to find other possible explanations of the variation we see in future research, i.e. phonological similarity.
