...
However, if we *plot* the predictions of the linear model, however (@fig-lm), it becomes immediately clear that using two linear predictors for spatial data -- in this case *virtual* spatial data -- is not ideal[^gaps]. Note that the data points eliminated in the elastic net regression are not part of the model.
